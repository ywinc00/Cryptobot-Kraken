// api/kraken-trade.js — Proxy para Kraken Futures API privada (trading real)
// Firma las peticiones con las credenciales del usuario.
...
export default async function handler(req, res) {
  ...
  if (!krakenApiKey || !krakenSecret) {
    return res.status(401).json({ error: 'Kraken API key y secret requeridos' });
  }
  ...
  try {
    ...
    console.error('Kraken trade proxy error:', err);
